...
typedef void (*psort)(vector<int>::iterator, vector<int>::iterator, greater<int>);
...
int main(int argc, char* argv[]) {
  ...
  std::sort(copyVec.begin(), copyVec.end(), greater<int>());
  ...
  for (int i = 0; i < num_sorts; i++) {
    copyVec = aVec;
    start = clock();
    sorts[i](copyVec.begin(), copyVec.end(), greater<int>());
